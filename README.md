...
| **编辑体验** | 块类型（标题/列表/待办/代码/图片）、`/` 菜单、Markdown 快捷输入、**块拖拽排序**、图片粘贴（自动压缩）、全文搜索（Ctrl+F 高亮跳转）、大纲导航、快照恢复与对比、四格式导出 + **.md 导入**、撤销/重做按钮 |
| **协作感知** | 块级评论线程（实时推送 + 未读角标 + 持久化已读 + **桌面通知**）、在线用户列表、连接/版本/待同步状态栏 |
