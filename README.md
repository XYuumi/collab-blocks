...
| **文档与权限** | 多文档管理、**编辑链接 / 只读链接**双轨分享、**三档权限选择器**（开放 / 登录可编辑 / 受限）、**协作者名单**（按用户名邀请）、用户注册/登录（用户名全局唯一）、回收站（7 天恢复） |
| **编辑体验** | 块类型（标题/列表/待办/代码/图片）、`/` 菜单、Markdown 快捷输入、**块拖拽排序**、图片粘贴（自动压缩）、全文搜索（Ctrl+F 高亮跳转）、大纲导航、快照恢复与对比、四格式导出 + **.md/.txt 导入**、**文档模板**、**多选块批量操作**（Shift+点击/↑↓）、**文档 emoji 图标**、**权限申请审批**、撤销/重做按钮、**@提及**、大文档**虚拟滚动** |
| **协作感知** | 块级评论线程（实时推送 + 未读角标 + 持久化已读 + **@提及** + **桌面通知**）、在线用户列表、连接/版本/待同步状态栏 |
