...
基于 DOM 渲染的块结构协同编辑器，像一个小型腾讯文档：**登录后创建文档、生成编辑/只读两种分享链接**；多人同时打开同一文档，输入/删除实时同步，带远程光标、块锁、块类型（标题/列表/待办/代码块 + `/` 菜单 + Markdown 快捷输入）、断线重连补发、冲突自动合并、Undo/Redo、版本与快照（可一键恢复到历史版本）、关页后未同步编辑恢复、大纲导航、明暗主题（无闪烁）、**手机浏览器响应式适配**。**不使用 OT/CRDT**，用「版本 + 块级 CAS + 拒绝后自动重试」保证收敛。数据与用户存于 SQLite。
...
npm test         # 40 个测试：引擎/WS 协议/认证存储/多文档权限 + 客户端双模型收敛
npm run build    # 构建前端
...
- **测试**：node:test ×40（引擎单元 / WS 集成 / 认证与存储 / 多文档与权限矩阵 / 客户端模型双端收敛）。
